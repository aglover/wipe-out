...
if !token or !excelFile
	throw new Error('You must provide an APP47_API_TOKEN and pass in a path to an excel file')
...
excel excelFile, (err, stuff) ->
	for row, index in stuff
		if index > 0
			email = row[1].trim()
			client.get("#{baseURL}/api/users/email/#{email.replace(/\./g, '%2E')}", httpHeader).on '2XX', (data, response) ->
				if data['_id']
